...
import com.flink.market_analysis.beans.AdClickEvent;
import com.flink.market_analysis.beans.AdCountViewByProvince;
import com.flink.market_analysis.beans.BlackListUserWarning;
...
public class AdStatisticsByProvince {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<AdCountViewByProvince> adCountResultStream = filterAdClickStream
                ...
                .timeWindow(Time.hours(1), Time.minutes(5))  //定义滑动窗口，5分钟输出一次
    ...
    public static class FilterBlackListUser extends KeyedProcessFunction<Tuple, AdClickEvent, AdClickEvent> {
        ...
        public void processElement(AdClickEvent value, Context ctx, Collector<AdClickEvent> out) throws Exception {
            //判断当前用户对同一广告的点击次数，如果不够上限，就 count+1,如果达到上限就直接过滤掉，并侧输出流报警
            //首先获取当前的count值
            Long curCount = countState.value();

            //1.判断是否是第一个数据，如果是的话，注册一个第二天0点的定时器
            if(curCount == 0 ){
                //ctx.timerService().currentProcessingTime() UTC伦敦是时间，北京时间是东八区
